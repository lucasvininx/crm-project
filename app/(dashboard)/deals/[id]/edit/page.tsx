...
export default function EditDealPage({ params }: EditDealPageProps) {
      ...
      try {
        ...
        if (dealData) {
          setFormData({
            ...
            value: dealData.value ? dealData.value.toString() : "",
            status: dealData.status || "novo",
            notes: dealData.notes || "",
          })

          if (dealData.expected_close_date) {
            setDate(new Date(dealData.expected_close_date))
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error.message || "Erro ao atualizar negócio")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-3xl font-bold tracking-tight">Editar Negócio</h1>
      </div>
      ...
      <Card>
        <form onSubmit={handleSubmit}>
          <CardHeader>
            ...
            <CardDescription>Atualize os dados do negócio.</CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between">
            <Button variant="outline" type="button" asChild>
              <Link href={`/deals/${params.id}`}>Cancelar</Link>
            </Button>
